...
class Proxy_pool():
    ...
    def __init__(self):
        print(os.getcwd())
        config = open(abs_path+ "\\config.yaml", mode="r", encoding="utf-8")
        cfg = config.read()
    ...
    def delete_proxy(self, proxy):
        requests.get(
            "http://{host}:{port}/delete/?proxypool={proxy}".format(host=self.host, port=self.port, proxy=proxy))
    ...
    def get_response(self, url, headers, https=False, cookie="", retry_count=5):
        if https:
            ...
            print(is_https)
        proxy = self.get_proxy().get("proxypool")
        while retry_count > 0:
            try:
