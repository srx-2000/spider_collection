...
class ThreadPool(object):
    ...
    def is_free_pool(self):
        if len(self.free_list)==self.max_num:
            return True
        else: return False
    ...
    def call(self):
        ...
        while event != StopEvent: #如果不为停止信号
            ...
            try:
                ...
                pool_status=self.is_free_pool()
                pool_free_num= len(self.generate_list)
            ...
            if callback is not None: # 是否有回调函数
                try:
                    callback(status,pool_status,pool_free_num,result) #执行回调函数
                except Exception as e:
                    pass
